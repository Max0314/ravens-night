...
export function GramophonePlayer() {
  ...
  return <aside className={`gramophone${open ? " is-open" : ""}`} aria-label="背景音乐">
    ...
    <button className="gramophone__toggle" type="button" aria-label={open ? "收起音乐" : "打开留声机"} aria-expanded={open} onClick={() => setOpen((current) => !current)}><span aria-hidden="true">◉</span><span>{open ? "收起音乐" : "留声机"}</span></button>
  </aside>;
}
