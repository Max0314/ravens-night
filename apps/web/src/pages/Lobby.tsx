...
export function Lobby({ room, onBegin, onTutorial, onRoles, canBegin, busy, error }: { room: RoomView; onBegin: () => void; onTutorial: () => void; onRoles: () => void; canBegin: boolean; busy: boolean; error?: string }) {
  ...
  return <main className="lobby"><section className="lobby__heading"><p>房间邀请码</p><h1>{room.code}</h1><p>{players.length}/{room.playerCount} 位玩家已入座</p><div className="lobby__guide-links"><button type="button" onClick={onTutorial}>教程</button><button type="button" onClick={onRoles}>角色表</button></div></section><SeatRing seats={seats} /><Panel className="lobby__status"><h2>{players.length === room.playerCount ? "所有人都已抵达" : "等待其他村民…"}</h2><p>让大家扫描二维码，或访问本站后输入相同邀请码。电视电脑请选择“公共大屏”。</p><InviteTools code={room.code} />{error ? <p className="form-error" role="alert">{error}</p> : null}{canBegin ? <Button onClick={onBegin} disabled={players.length < room.playerCount || busy}>{busy ? "正在敲响钟声…" : "开始新手教学"}</Button> : <p className="lobby__waiting">由创建者开始教学</p>}</Panel></main>;
}
